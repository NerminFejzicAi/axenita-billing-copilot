...
### Formalno zatvaranje pod-gatea `P5-I4A` (D-076, 2026-08-30)

**Ova sekcija je aditivna.** Nijedna ugovorna klauzula §7.5a.1–§7.5a.3, tabela segmentacije,
sekcija D-073, sekcija D-074 ni pomirenje D-075 iznad se **ne prepisuje**; sve ostaju **doslovno na
snazi**. D-076 je **činjenično post-merge pomirenje** i **formalno zatvaranje** pod-gatea `P5-I4A`.
...
P5-I4A FORMAL CLOSURE             = YES   (uslovno efektivno, vidi nize)
```

**Kanonski dokazni blok:**
...
**Verifikacijski stack** — `typecheck` `PASS`, `lint` `PASS`, `test` `PASS` (40 / 967),
`test:e2e` `PASS` (5 / 41), `test:integration` `PASS` (4 / 46), `test:security` `PASS` (22 / 813);
agregat **71 fajl / 1867 testova / 0 padova / 0 preskoka**. **Provenijencija je razdvojena** u
D-076 (`06`): poruka implementacijskog commita atestira pet imenovanih traka bez brojeva, a
kompletan šestonivovski stack sa brojevima je reprodukovan nezavisnim vlasničkim reviewom i
post-publikacionom verifikacijom.

**Odluka o zatvaranju je donesena, ali zatvaranje NIJE odmah efektivno.** Ono postaje efektivno tek
nakon što D-076 sam bude autorstvom dovršen, nezavisno vlasnički pregledan, vlasnički prihvaćen,
...
**Formulacije iznad** koje glase `P5-I4A IMPLEMENTATION AUTHORIZED = NO`,
`P5-I4A IMPLEMENTATION STARTED = NO`, `P5-I4A IMPLEMENTATION EXECUTION = PROHIBITED` i
„implementacijski kandidat `P5-I4A` ostaje **nekanonski**" opisuju **pred-D-076 stanje**,
...
**`CONFORMANT_AND_NON_CIRCULAR`**. **Nijedan nalaz ne autorizuje izmjenu koda, granta, scheme,
testa ni ugovora**, i **nijedan nije promovisan u bloker**. Puna tabela je u D-076 (`06`).
...
**Status narednog pod-gatea:**

```text
P5_I4B_DEPENDENCIES_SATISFIED                          = YES
P5_I4B_CONTRACT_DETERMINISTIC                          = YES
P5_I4B_OWNER_DECISIONS_REQUIRED                        = 0
P5_I4B_IMPLEMENTATION_AUTHORIZATION_PREFLIGHT_ELIGIBLE = YES  (tek post-D-076-kanonski)
P5_I4B_IMPLEMENTATION_AUTHORIZED                       = NO
P5_I4B_IMPLEMENTATION_STARTED                          = NO
```

**Podobnost nije autorizacija.** Obuhvat `P5-I4B` iz tabele segmentacije iznad se **ne proširuje i
ne reinterpretira**; obavezni su zaseban implementacijski autorizacijski preflight i zaseban
vlasnički autorizacijski potez.
...
**Roditeljski `P5-I4` ostaje `INCOMPLETE` / `OPEN`**; `P5-I5` ostaje `STILL DEPENDENCY-BLOCKED` —
**D-076 ga ne odblokira**, jer `P5-I5` zavisi od **kompletnog** `P5-I4`. Checklist Faze 5 ostaje
**`49 / 14`** uz **`PHASE5_CHECKBOX_TRANSITIONS = 0`**; sedamnaest forecast redova ostaje
**neoznačeno**, a **`EXPECTED_POST_P5_I4_CLOSURE_CHECKLIST = 49 / 31`**. `CO-P5-I3-I4-1` i
...
#### Formalno zatvaranje pod-gatea `P5-I4B` (D-078, 2026-09-01)

**Ova sekcija je aditivna.** Nijedna ugovorna klauzula §7.5a, tabela segmentacije, sekcija D-073,
sekcija D-074, pomirenje D-075, zatvaranje D-076 ni autorizacijska sekcija D-077 iznad se **ne
prepisuje**; sve ostaju **doslovno na snazi**. D-078 je **činjenično post-merge pomirenje** i
**formalno zatvaranje** pod-gatea `P5-I4B`.
...
P5-I4B FORMAL CLOSURE             = YES   (uslovno efektivno, vidi nize)
```
...
**Higijenska remedijacija `VE-1` — izvedena PRIJE ovog zatvaranja:**
...
VE-1                    CLOSED / NON-ATTRIBUTABLE TO P5-I4B
```

`VE-1` je bio **defekt portabilnosti sigurnosnog testa** pod `core.autocrlf=true`, **ne** defekt
`P5-I4B` i **ne** defekt Prisma modela. Popravka je bila **jedan fajl, jedna linija**. **`VE-1` se
ovim zapisom ne otvara ponovo** i **nikakva dodatna remedijacija nije autorizovana.**

**Verifikacijski stack nad `9145abc6…`** — `typecheck` `PASS`, `lint` `PASS`, `test` `PASS`
(44 / 1119), `test:e2e` `PASS` (5 / 41), `test:integration` `PASS` (4 / 46), `test:security` `PASS`
(22 / 813); agregat **75 fajlova / 2019 testova / 0 padova / 0 preskoka**, uz sigurnosnu traku
**813 / 813 `PASS`**. Post-publikacioni negativni dokaz `VE-1` je **5 / 5 `PASS`**.

**Odluka o zatvaranju je donesena, ali zatvaranje NIJE odmah efektivno.** Ono postaje efektivno tek
nakon što D-078 sam bude autorstvom dovršen, nezavisno vlasnički pregledan, vlasnički prihvaćen,
objavljen/merged, kanonski na `origin/main` i post-publikaciono verifikovan.
...
**Formulacije iznad** koje glase `P5-I4B IMPLEMENTATION AUTHORIZATION EFFECTIVE = NO`,
`P5-I4B IMPLEMENTATION EXECUTION ELIGIBLE = NO` i `P5-I4B IMPLEMENTATION STARTED = NO` opisuju
**pred-D-078 stanje**, **historijski su tačne** i **ne prepisuju se**; **mjerodavan je ovaj statusni
model.**

**Dispozicija closure nalaza.** `OD-P5-I4B-C1` = **`APPROVED`** (zaseban closure zapis = D-078);
`OD-P5-I4B-C2` = **`APPROVED`** (nezavisni closure review prethodio, ishod `PASS`);
`OD-P5-I4B-C3` = **`APPROVED`** (zatvaranje `P5-I4B` se **ne kombinuje** sa autorizacijom
`P5-I4C`); `OD-P5-I4B-C4` = **`NON_BLOCKING_HOLD_AS_AUTHORIZED` / `CARRIED FORWARD`** za
`POST /exports/{exportJobId}/retry`, razlog **`NO_CANONICAL_REQUEST_BODY_DEFINED_IN_03`**;
`OD-P5-I4B-C5` = **`CONFORMANT` / `NO GAP`** za odsustvo Nest module wiringa, **bez remedijacije**;
`RFC-B-COUNT` = **`INFORMATIONAL / ACCEPTED`**; `VE-1` = **`CLOSED`**; `FMT-1` =
**`INFORMATIONAL / ACCEPTED AS-IS`**. **Nijedan nalaz ne autorizuje izmjenu koda, granta, scheme,
testa ni ugovora**, i **nijedan nije promovisan u bloker**. Puna tabela je u D-078 (`06`).

**`RFC 8785 Appendix B` pinovanih brojčanih vektora je `13`.** Ranija radna brojka **`24` je
superseidirana i nije mjerodavna.**
...
**Podobnost nije autorizacija.** Obuhvat `P5-I4C` iz tabele segmentacije iznad se **ne proširuje i
ne reinterpretira**; obavezni su zaseban implementacijski autorizacijski preflight i zaseban
vlasnički autorizacijski potez — vjerovatno zaseban autorizacijski zapis **`D-079`**, ako `D-079`
ostane slobodan. **D-078 ne izvodi nijedan od njih.**
...
**Roditeljski `P5-I4` ostaje `INCOMPLETE` / `OPEN`**; `P5-I5` ostaje `STILL DEPENDENCY-BLOCKED` —
**D-078 ga ne odblokira**, jer `P5-I5` zavisi od **kompletnog** `P5-I4`. Checklist Faze 5 ostaje
**`49 / 14`** uz **`PHASE5_CHECKBOX_TRANSITIONS = 0`**; sedamnaest forecast redova ostaje
**neoznačeno**, a **`EXPECTED_POST_P5_I4_CLOSURE_CHECKLIST = 49 / 31`**. `CO-P5-I3-I4-1` i
`CO-P5-I3-I4-2` ostaju **neispunjeni** ne-checkbox kriteriji prihvatanja roditeljskog `P5-I4`. Vidi
D-078 u `06`, `03` §4.1, `05` §6 i `08` §12.11.

#### Vlasnička autorizacija implementacije `P5-I4C` (D-079, 2026-09-02)

**Nijedan pasus, tabela ni anotacija iznad se ne prepisuje.** D-079 je **vlasnički ratifikovan
governance zapis autorizacije implementacije `P5-I4C`** i **dokumentacija isključivo**. On
konstatuje **pet ugovornih vlasničkih odluka** koje zatvaraju posljednje interpretativne
ambiguitete `P5-I4C`, i **jednu autorizacijsku odluku**. **Nijedna linija koda, testa, fixturea,
scheme, migracije, granta ni zavisnosti se ovom odlukom ne uvodi**, i **nijedna baza nije
kontaktirana**.
...
**`OD-P5-I4C-1` — kanonski literal `idempotency_keys.endpoint`.**
...
Perzistirana vrijednost je **kanonska obavezna spelling endpointa iz `03` §4, doslovno**. **Ne
izvodi se iz runtime mount putanje**, **ne perzistira se kao `/api/v1/patient-references`** i **ne
zamjenjuje se simboličkim identifikatorom operacije**. Pravilo je **opšte**, ne jednokratno.
...
Poruka je **statična**; **poslani ključ se nikada ne odražava**; **Unicode izvan navedenog raspona,
kontrolni znakovi i whitespace su nevalidni**. **Nijedan novi error kod se ne uvodi** — `400
VALIDATION_ERROR` za prisutan-ali-neprihvaćen header je već kanonski (D-055, klauzula 12; `03` §9).
Validacija je **provjera headera** i prethodi scope inspekciji, advisory locku i hashiranju tijela.

**`OD-P5-I4C-3` — tačno bajtno enkodiranje advisory locka.** Ovo **zamjenjuje neodređenost**
formulacije „length-prefixed UTF-8 reprezentacija scopea" iz *Modela idempotencijske transakcije*
iznad **tačnom specifikacijom**; sama ta formulacija ostaje **tačna i nepromijenjena**.
...
perzistencija / logiranje kljuca  = ZABRANJENO
```

`length(UTF8(X))` je broj **UTF-8 bajtova**. `endpoint` komponenta je **ista vrijednost** iz
`OD-P5-I4C-1`. **Komponenti scopea u preimageu je i dalje TAČNO ČETIRI**; `idem-lock-v1` je
**konstantan domen separator, ne peta komponenta scopea**, pa je klauzula D-072 o „tačno četiri
komponente scopea" **ispunjena, a ne izmijenjena**, i **svih pet koraka izvođenja iz D-072 ostaje
doslovno na snazi**. Prihvaćeni domen `Idempotency-Key`-a je **isključivo ASCII**, pa je granica od
**255 UTF-8 bajtova doslovno granica postojeće kolone `varchar(255)`** (`02` §15.2); **nikakva
izmjena scheme nije potrebna ni autorizovana**. **Tačni bajtovi i očekivani `int64` MORAJU biti pinovani doslovnim vektorima**
(`08` §12.12, obaveza 6). **Advisory lock ostaje kontrola konkurencije, ne sigurnosna ni
autorizacijska granica** (`09` §4.2).
...
VALIDATION_ERROR`** sa **generičkom statičnom porukom koja ne citira vrijednost**. **Postojeća
kanonska nullable/opciona semantika se čuva**, i **polje se ne čini tiho obaveznim**. **Bez
proširenja u FHIR `AdministrativeGender`.** Zatvaranje vokabulara je **aplikacijsko** — `02` za
`sex_code` **ne definiše DB enum ni `CHECK`** — pa se **nikakav DB artefakt ne mijenja**.

**`OD-P5-I4C-5` — aplikacijska validacija `birthYear`-a.** **Integer**, **minimum `1900`**,
**maksimum `2200`**, **inkluzivno**, uz **očuvanu** nullable/opcionu semantiku. Povreda daje
**`422 VALIDATION_ERROR`** sa **generičkom statičnom porukom**, uz **nula DB round-tripova za
nevalidan aplikacijski ulaz**; **`SQLSTATE 23514` se ne koristi kao normalna validacija**.
Postojeći `patient_references_birth_year_check` ostaje **nepromijenjen** i **posljednja odbrambena
linija**.

**Autorizovan izvršivi obuhvat** je **tačno** onaj iz reda `P5-I4C` u tabeli segmentacije iznad —
**`idempotency, audit writer and POST`** — u trinaest stavki: idempotency servis; advisory-lock
kontrola konkurencije; audit writer; `POST /patient-references`; konzumacija `MANUAL` v1
normalizacije; perzistencija HMAC-a kao `external_patient_ref_hash`; ciljani retry pseudonima;
`PATIENT_REFERENCE_ALREADY_EXISTS`; servisni lookup po pseudonimu; servisni lookup po eksternoj
referenci; `CO-P5-I3-I4-1`; `CO-P5-I3-I4-2`; integracioni / sigurnosni / API / concurrency dokazi.
**Izvan obuhvata** ostaju: nova HTTP lookup ruta; encounter površina; document površina; redakcija;
encryption envelope write-back; izmjena scheme; migracije; izmjena RLS-a; izmjena grantova; izmjena
runtime zavisnosti; izmjena lockfilea. **Oba servisna lookupa su servisne sposobnosti bez HTTP
površine; nijedna nova ruta se ne registruje.** **`P5-I4C` smije konzumirati čiste `P5-I4B` hash
funkcije direktnim importom, i Nest provider wiring se NE uvodi radi „popravke" njegovog
odsustva** (D-078, `OD-P5-I4B-C5`).

**Zamrznuti mutacijski predikati `P5-I4C`:**
...
**Ako implementacija naknadno ustanovi da je bilo koji predikat zapravo `YES`, MORA STATI (`HOLD`)
i vratiti se u governance.** **Nikakvo tiho proširenje obuhvata.**

**Prenesene dispozicije ostaju nepromijenjene:** `POST /exports/{exportJobId}/retry` request-hash
vektor **`NON_BLOCKING_HOLD_AS_AUTHORIZED` / `CARRIED FORWARD`**
(`NO_CANONICAL_REQUEST_BODY_DEFINED_IN_03`); odsustvo Nest module wiringa **`CONFORMANT` / `NO
GAP`**; `RFC 8785 Appendix B` pinovanih brojčanih vektora **`13`**, ne `24`; `VE-1` **`CLOSED`** i
**ne otvara se ponovo**; poznato Prettier odstupanje **`INFORMATIONAL / ACCEPTED AS-IS`**, bez
drive-by popravke.

**Odluka je donesena, ali autorizacija NIJE odmah efektivna.** Ona postaje efektivna tek nakon što
D-079 sam bude autorstvom dovršen, nezavisno vlasnički pregledan, vlasnički prihvaćen,
objavljen/merged, kanonski na `origin/main` i post-publikaciono verifikovan. **Lokalni,
nepublikovani commit autorstva D-079 NE čini autorizaciju efektivnom**, i **i nakon efektivnosti**
implementacija smije početi **isključivo kroz zaseban `P5-I4C` gate izvršenja**.

**Formulacije iznad** koje glase `P5-I4C IMPLEMENTATION AUTHORIZED = NO` i „podobnost nije
autorizacija" opisuju **pred-D-079 stanje**, **historijski su tačne** i **ne prepisuju se**;
**mjerodavan je ovaj statusni model.**
...
`P5-I4`; nakon `P5-I4C` slijedi **zaseban gate zatvaranja roditeljskog gatea**. **`P5-I5` ostaje
`STILL DEPENDENCY-BLOCKED`**, jer zavisi od **kompletnog i formalno zatvorenog** `P5-I4`. Checklist
Faze 5 ostaje **`49 / 14`** uz **`PHASE5_CHECKBOX_TRANSITIONS = 0`**; sedamnaest forecast redova
ostaje **neoznačeno**, a **`EXPECTED_POST_P5_I4_CLOSURE_CHECKLIST = 49 / 31` je isključivo
forecast**. `CO-P5-I3-I4-1` i `CO-P5-I3-I4-2` ostaju **ne-checkbox kriteriji prihvatanja**
roditeljskog `P5-I4`. Vidi D-079 u `06`, `03` §4.1, `05` §6 i `08` §12.12.

#### Formalno zatvaranje pod-gatea `P5-I4C` (D-080, 2026-09-05)

**Ova sekcija je aditivna.** Nijedna ugovorna klauzula §7.5a, tabela segmentacije, sekcija D-073,
sekcija D-074, pomirenje D-075, zatvaranje D-076, autorizacijska sekcija D-077, zatvaranje D-078 ni
autorizacijska sekcija D-079 iznad se **ne prepisuje**; sve ostaju **doslovno na snazi**. D-080 je
**činjenično post-merge pomirenje** i **formalno zatvaranje** pod-gatea `P5-I4C`.
...
P5-I4C FORMAL CLOSURE             = YES   (uslovno efektivno, vidi nize)
```

**Kanonski dokazni blok implementacije:**
...
**Kanonski dokazni blok higijenske remedijacije sirovog `NUL`-a — izvedena PRIJE ovog zatvaranja:**
...
HYGIENE PATHS           1
PUBLICATION DRIFT       ZERO
HYGIENE STATUS          INDEPENDENTLY REVIEWED / OWNER-ACCEPTED / PUBLISHED /
                        MERGED / CANONICAL / POST-PUBLICATION VERIFIED
```

Higijenski nalaz je bio **defekt repozitorijske higijene i tekstualne pregledljivosti**, ne defekt
ponašanja: jedan sirovi `U+0000` bajt kao separator u `stubHmac()` helperu speca
`apps/api/src/patient-reference/application/patient-reference-create.service.spec.ts` činio je taj
tekstualni fajl **binarnim** za `git grep` i `git diff`. Popravka je bila **jedan fajl, jedna
fizička linija**, uz **runtime bajt-identičan** tekstualni escape; **runtime semantika je
očuvana**. **Nalaz se ovim zapisom ne otvara ponovo** i **nikakva dodatna remedijacija nije
autorizovana.**

**Vlasnički closure uslovi:**
...
**Autorizovan izvršivi obuhvat je ispunjen u cijelosti** — svih **trinaest** stavki D-079
`RULING B`: idempotency servis; advisory-lock kontrola konkurencije; audit writer;
`POST /patient-references`; konzumacija `MANUAL` v1 normalizacije; perzistencija HMAC-a kao
`external_patient_ref_hash`; ciljani retry pseudonima; `PATIENT_REFERENCE_ALREADY_EXISTS`; servisni
lookup po pseudonimu; servisni lookup po eksternoj referenci; `CO-P5-I3-I4-1`; `CO-P5-I3-I4-2`;
integracioni / sigurnosni / API / concurrency dokazi.
...
**Prihvaćeni verifikacijski stack `P5-I4C`** — `typecheck` `PASS`, `lint` `PASS`, `test` `PASS`
(49 / 1259), `test:e2e` `PASS` (5 / 41), `test:integration` `PASS` (4 / 46), `test:security` `PASS`
(23 / 851); agregat **81 fajl / 2197 testova / 2197 `PASS` / 0 padova / 0 preskoka**. Prethodna
osnova (D-078) bila je `75 / 2019`; delta je **`+6 fajlova / +178 testova`**, i **nijedan postojeći
test nije nestao**.

**Zasebna higijenska verifikacija** — pogođeni spec **62 / 62 `PASS`**, `P5-I4C` ciljani unit set
**154 / 154 `PASS`**, puna unit svita **1259 / 1259 `PASS`**, `typecheck` `PASS`, `lint` `PASS`,
**diferencijalni** format `PASS`. **Agregat `81 / 2197` NIJE ponovo izvršen nakon higijene**, i
**e2e / integration / security trake NISU rerunovane tokom higijenskog gatea**; originalni
prihvaćeni dokaz ostaje važeći jer je higijena promijenila isključivo test-source reprezentaciju uz
očuvanu runtime semantiku, a izmijenjeni artefakt je nezavisno potvrđen.

**Odluka o zatvaranju je donesena, ali zatvaranje NIJE odmah efektivno.** Ono postaje efektivno tek
nakon što D-080 sam bude autorstvom dovršen, nezavisno vlasnički pregledan, eksplicitno vlasnički
prihvaćen, objavljen/merged, kanonski na `origin/main` i post-publikaciono verifikovan.
...
**Formulacije iznad** koje glase `P5-I4C IMPLEMENTATION AUTHORIZATION EFFECTIVE = NO`,
`P5-I4C IMPLEMENTATION EXECUTION ELIGIBLE = NO`, `P5-I4C IMPLEMENTATION STARTED = NO` i
„`POST /patient-references` nije implementiran" opisuju **pred-D-080 stanje**, **historijski su
...
`ACCEPTED` / `NON-BLOCKING`**; konformansna restauracija TypeScript ogledala `03` §8 kataloga =
**`ERROR_CODE_MIRROR_CONFORMANCE_RESTORATION_CONFIRMED = CONFIRMED`** — kod je u §8 kanonski od
D-072, `409` mapiranje u §8.1 je kanonsko, pozicija je identična, i **nijedan novi error kod nije
uveden**; `P5-I4C` sirovi `NUL` = **`RESOLVED` / `CANONICAL` / `NON-BLOCKING`**; sirovi `NUL` u
`apps/api/src/identity/domain/permission-matrix.ts` = **`PRE-EXISTING` / `OUT OF P5-I4C SCOPE` /
`UNCHANGED` / `SEPARATE NON-BLOCKING FOLLOW-UP`**, i **D-080 ga NE remedira**; `GOV-DRIFT` =
**`RECONCILED`**. **Nijedan nalaz ne autorizuje izmjenu koda, granta, scheme, testa ni ugovora**, i
**nijedan nije promovisan u bloker**. Puna tabela je u D-080 (`06`). Poznato Prettier odstupanje
Faze 4 ostaje **`INFORMATIONAL / ACCEPTED AS-IS`**, **bez drive-by popravke**.
...
`P5-I4C` je **treći i posljednji** pod-gate `P5-I4`, pa **nema `P5-I4D`**. Nakon što zatvaranje
`P5-I4C` postane efektivno, naredni lifecycle gate je **zaseban gate formalnog zatvaranja
roditeljskog gatea `P5-I4`** — **ne `P5-I5`**. Taj gate je jedini koji smije adresirati sedamnaest
forecast redova i `CO-P5-I3-I4-1` / `CO-P5-I3-I4-2` kao kriterije prihvatanja roditelja.
...
**Roditeljski `P5-I4` ostaje `INCOMPLETE` / `OPEN`** — **zatvaranje `P5-I4C` ga NE zatvara
automatski** i **ne prevodi automatski sedamnaest roditeljskih forecast redova**; `P5-I5` ostaje
`STILL DEPENDENCY-BLOCKED` — **D-080 ga ne odblokira**, jer `P5-I5` zavisi od **kompletnog i
formalno zatvorenog** `P5-I4`. Checklist Faze 5 ostaje **`49 / 14`** uz
**`PHASE5_CHECKBOX_TRANSITIONS = 0`** i **`P5-I4C_CLOSURE_CHECKBOX_TRANSITIONS = 0`**; sedamnaest
forecast redova ostaje **neoznačeno**, a **`EXPECTED_POST_P5_I4_CLOSURE_CHECKLIST = 49 / 31` je
isključivo forecast** i **nije tekuće stanje**. `CO-P5-I3-I4-1` i `CO-P5-I3-I4-2` su **ispunjeni
obuhvatom `P5-I4C`**, ali ostaju **ne-checkbox kriteriji prihvatanja** roditeljskog `P5-I4` i **ne
