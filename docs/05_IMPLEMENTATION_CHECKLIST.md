...
**REDOSLIJED IZVRŠENJA I VLASNIŠTVO CROSS-CUTTINGA (D-069, 2026-08-27) — nijedan pasus iznad se ne
prepisuje, i nijedna kućica se ne mijenja.** **Checklist Faze 5 ostaje `49 / 9`.**

**Tekući kanonski redoslijed izvršenja je `P5-I3 → P5-I4 → P5-I5`**, jer kolona zavisnosti iz
`04` §7.5 (D-062) **ostaje mjerodavna i nikada nije waivovana**. Formulacija D-068 „naredni
obavezni gate je `P5-I5`" je **planska statusna tvrdnja**, tačna na dan svog zapisa, i **za
tekuće sekvenciranje izvršenja je supersedirana**; „podobnost `P5-I5`" znači **podoban nakon
ispunjenja deklarisanih zavisnosti**, ne ovlašten da preskoči `P5-I3` i `P5-I4`.
...
**`P5-I3` traži zaseban read-only preflight i zasebnu vlasničku autorizaciju; D-069 ga ne
autorizuje.**

**`P5-I4` ostaje slice `patient_references`**, i **dodatno posjeduje** — kao **prvi kanonski
konzument** — **idempotency servis**, **audit writer**, kanonizaciju **`request_sha256`**, **audit
self-hash** i **konkretan `TenantDatabaseService` facade** (uslovna obaveza D-056, čiji je trigger
time dobio **imenovanog vlasnika**). **To nije novi slice**, nego **podržavajuće obaveze `P5-I4`**.
**Devet facade redova niže OSTAJE NEOZNAČENO** i ostaje neoznačeno dok `P5-I4` ne bude
implementiran, verifikovan, kanonski i formalno pomiren.

**`P5-I5` preflight:** kanonski read-only preflight `P5-I5` završio je ishodom
**`P5_I5_PREFLIGHT_HOLD_OWNER_DECISION_REQUIRED`** sa **pet** neriješenih vlasničkih odluka;
**vlasnik je ratifikovao svih pet (D-069)**, pa je **`OWNER_DECISIONS_REQUIRED_FOR_P5_I5 = 0`**.
...
**Anotacija (`P5-I2B-D`, 2026-08-25) — obje anotacije iznad se ne prepisuju.** Rečenica
„**`P5-I2B` time nije autorizovan**" je tačna **u odnosu na `P5-I2A`**, i ostaje nepromijenjena:
`P5-I2A` `P5-I2B` zaista nije autorizovao. **Vlasnik ga je autorizovao zasebnim potezom nakon
D-065**, i `P5-I2B` je danas **implementiran, nezavisno auditiran, merged i kanonski** — **PR
#36**, merge SHA `0e4d113f0eedddcd2db890180767768c5b422264` (D-066). **Prognoza `49 / 9` i dalje
je uslovljena kompletnim, verifikovanim i kanonskim `P5-I2`, ne samim `P5-I2B`-om**; `P5-I2C` i
...
**IMENOVANI VLASNIK USLOVNOG TRIGGERA (D-069, 2026-08-27) — tekst obaveze iznad i devet redova
niže se NE mijenjaju, i NIJEDAN red se sada ne označava.**

**Uslovni trigger D-056 sada ima imenovanog budućeg vlasnika: `P5-I4`.**

`P5-I4` je **prvi kanonski Faza-5 tenant business modul** koji traži da se domenski/poslovni
iskazi izvršavaju nad **već otvorenom, pinovanom tenant transakcijom** — tačno stanje koje D-056,
dio A opisuje rečenicom „**kada stvarni tenant business repozitorij/modul zatraži tu
apstrakciju**". **Dolazak broja faze i dalje sam po sebi obavezu ne stvara**; obavezu stvara
`P5-I4` kao stvarni modul.

**Devet obaveza niže postaju kriteriji prihvatanja u vlasništvu `P5-I4`.** Nijedna nije uklonjena,
oslabljena, preformulisana ni označena završenom. **Sve ostaju `[ ]`**, i ostaju neoznačene dok
`P5-I4` ne bude **implementiran, verifikovan, kanonski i formalno pomiren**.

**Nijedan facade kod nije ovlašten odlukom D-069.** Uvođenje dummy klase ili stuba i dalje **nije
...
`NOT AUTHORIZED`**. *(**As-of-time zapis, 2026-08-24.** Tvrdnja o `P5-I2B` je tačna na dan ovog
bloka i ne prepisuje se; `P5-I2B` je od **2026-08-25** kanonski i formalno zatvoren — **D-066**,
**PR #36**. `P5-I2` u cjelini i dalje **ostaje `IN_PROGRESS` / `NOT COMPLETE`**.)*
...
*(**As-of-time zapis, 2026-08-24; ne prepisuje se.** Pravilo „`P5-I2A` ne autorizuje `P5-I2B`"
ostaje trajno na snazi i **jeste** ispoštovano: `P5-I2B` je autorizovan **zasebnim vlasničkim
potezom** nakon D-065 i danas je kanonski — **D-066**, **PR #36**. **Nepromijenjeno ostaje:**
...
**Anotacija (`P5-I2B-D`, 2026-08-25) — oba pasusa iznad se ne prepisuju.** Ona bilježe stanje
**u trenutku pomirenja D-065, prije autorizacije i implementacije**, i tačna su kao takva.
**Propisana sekvenca je u međuvremenu izvršena u cijelosti:** preflight je ponovljen nad
kanonskim `main`-om, vlasnik je **zasebnim potezom** autorizovao pod-gate, implementacija je
izvedena i **nezavisno auditirana**, a auditirani commit je merged **nepromijenjen**. **Tekući
status `P5-I2B` je `IMPLEMENTED` / `AUDITED` / `MERGED` / `CANONICAL` / formalno zatvoren**
(**D-066**) — vidi blok `Pod-gate P5-I2B — Faza-5 slice paketa 013` ispod. **Nepromijenjeno
ostaje:** `P5-I2C` = `NOT IMPLEMENTED` / `NOT AUTHORIZED`; `P5-I2V` / `★` = `NOT EXECUTED`;
...
`RULING 2`**, u obuhvatu koji je vlasnik autorizovao za **isključivo** pod-gate `P5-I2B`.
**Ovo je kanonsko stanje.** Grana je gurana, **PR #36** je merged u kanonski `main`.
**Formalno zatvaranje je zapisano odlukom D-066.**

```text
STATUS:                 IMPLEMENTED / INDEPENDENTLY AUDITED / MERGED / CANONICAL / CLOSED
IMPLEMENTATION:         6efee207c9ca52a22ca2cdeb97773832931711e7
...
CANONICAL MIGRATION:    20260825013452_013_rls_policies_phase5
CANONICAL OWNER TEST:   apps/api/test/phase5-rls-grants.security.ts
FORMAL CLOSURE:         D-066
```

**Auditirani commit je merged nepromijenjen.** `6efee207` je `parent 2` merge commita `0e4d113f`
i njegov je predak; merge nije uveo nijednu izmjenu preko nezavisno auditiranog stanja.

**Dokazan obuhvat — rekonstruisan iz samih commitovanih artefakata, ne iz prethodne proze:**

- **jedna** Faza-5 sigurnosna migracija paketa `013` —
  `apps/api/prisma/migrations/20260825013452_013_rls_policies_phase5/migration.sql`, hronološki
  **šesta** i posljednja u kanonskom lancu;
- **tačno jedna eksplicitna transakcijska granica najvišeg nivoa** — **jedan** `BEGIN` i **jedan**
  `COMMIT` doslovno napisani u `migration.sql` (D-065, `RULING 2`; `02` §29.4a.0). **Bez
  međukoraka `COMMIT`** i **bez transakcijski prekidajućeg iskaza**;
- **`REVOKE ALL … FROM PUBLIC` nad svih sedam tabela, prije svakog granta** —
  `patient_references`, `encounters`, `encounter_diagnoses`, `storage_objects`,
  `encounter_documents`, `idempotency_keys`, `audit_events`;
- **`ENABLE` + `FORCE ROW LEVEL SECURITY` nad svih sedam** — čime schema prelazi na **13 / 13**
  `true`/`true`;
- **tačno 15 novih politika**, sve `TO copilot_app`, sve sa kanonskim tenant predikatom `02`
  §17.1 **doslovno i neoslabljeno**; ukupan katalog prelazi na **25** (`10 + 10 + 3 + 2`,
  D-065, `RULING 1`);
- **`storage_objects` zadržava nula politika** — `ENABLE` + `FORCE` bez ijedne politike je
  **default-deny**, i to **jeste** sigurnosna kontrola. Nula grantova, nula redova.

**Petnaest politika — tačna imena:**

```text
patient_references_select        encounter_documents_select
patient_references_insert        encounter_documents_insert
encounters_select                encounter_documents_update
encounters_insert                idempotency_keys_select
encounters_update                idempotency_keys_insert
encounter_diagnoses_select       idempotency_keys_update
encounter_diagnoses_insert       audit_events_select
                                 audit_events_insert
```

**Grant površina — tačno po D-064 / D-065, bez viška:**

- **table-level `SELECT` + `INSERT`** za `copilot_app` nad **šest** tabela;
- **column-level `UPDATE`** nad **tačno tri** tabele — `encounters` (**12** kolona),
  `encounter_documents` (**isključivo** `archived_at`) i `idempotency_keys` (`response_status`,
  `response_body`, `locked_at`, `completed_at`);
- **nijedan table-level `UPDATE`**, **nijedan `DELETE`**, **nijedan `TRUNCATE`**, **nijedna
  sekvencijalna privilegija**;
- **`audit_events` je append-only na nivou granta** (`02` §15.4, §19.2);
- **`storage_objects` ne dobija ništa**;
- **`copilot_system` = nula** i **`PUBLIC` = nula** nad svih sedam tabela;
- **`§23.4` maintenance allowlista NIJE proširena** — ostaje na **tačno šest** tabela
  (`02` §23.4.4b).

**Dokaz — vlasništvo i evolucija (D-064, `OD-9`):**

- **novi `apps/api/test/phase5-rls-grants.security.ts`** je **trajni vlasnik steady-state
  sigurnosnog kataloga**: statički dokaz eksplicitne transakcije, tačni katalozi RLS-a, politika,
  predikata, table i column grantova, negativne privilegijske površine, te behavioural dokazi —
  same-tenant allow, cross-tenant deny, fail-closed bez tenant konteksta, `UPDATE` nad uskraćenom
  kolonom na `42501`, i `storage_objects` nedosežan;
...
- **statički package-boundary ZERO-CAPABILITY dokaz paketa `003` je očuvan** (`OD-9`, dio A) —
  kao trajni statički scan nad vlastitim SQL-om migracije `003`;
- **statički package-boundary ZERO-CAPABILITY dokaz paketa `011` je očuvan** i nedirnut;
- **nijedna primijenjena migracija nije editovana.**

**Sigurnosna i obuhvatna isključenja `P5-I2B`:** nije uveden **nijedan** aplikacijski izvor,
**nijedna** izmjena `schema.prisma`, nijedan servis, ruta, kontroler ni DTO; **paket `014` nije
kreiran**; **`★` RI-naspram-RLS dokaz nije izvršen**; nijedna nova rola, nijedan `BYPASSRLS`,
nijedan `SECURITY DEFINER`, nijedna owner politika i nijedno proširenje allowliste.

**Šta ovaj pod-gate ne tvrdi.** Ne zatvara Fazu 5 i **ne zatvara `P5-I2`**; **ne autorizuje
`P5-I2C` ni `P5-I2V`**; ne izvršava i ne razrješava **`★`**; ne autorizuje `P5-I5`; ne zatvara
`D-OPEN-004a`; ne mijenja nijednu Faza-3/4 invarijantu; i **ne označava nijednu kućicu**.

**Aritmetika checklista — mehanički izvedena, ne pretpostavljena (D-066, `RULING C`).**
...
**Nijedna kućica se ne mijenja.** Jedini red koji `P5-I2` može označiti je **`Schema → RLS`**, a
njegov **ratifikovani kriterij prihvatanja** je **kompletan, verifikovan i kanonski `P5-I2`**
(D-064, *Prognoza checklista*; potvrđeno anotacijom `P5-I2A-C`) — ne pojedinačni pod-gate.
**`P5-I2C` i `P5-I2V` nisu izvršeni, pa antecedent nije ispunjen** i red ostaje **neoznačen**.
**`Tests → cross-tenant FK` se u `P5-I2` ne smije označiti** (D-064) — značenje tog reda uključuje
i kasnije API/`422` ponašanje koje posjeduje `P5-I5`. **Prognoza `49 / 9` ostaje prognoza.**

**Tekući status pod-gateova (2026-08-25, nakon D-066).**

```text
P5-I2A     CANONICAL                                   (PR #33)
P5-I2B     CANONICAL / FORMALLY CLOSED                 (PR #36)
P5-I2C     NOT IMPLEMENTED / NOT AUTHORIZED
P5-I2V/★   NOT EXECUTED
...
**ANOTACIJA TEKUĆEG STATUSA (D-069, 2026-08-27) — blok i pasus iznad se ne prepisuju; nijedna
kućica se ne mijenja, checklist ostaje `49 / 9`.**

**Tekući naredni implementacijski slice je `P5-I3`**, ne `P5-I5`. Zavisnosti koje pasus iznad i
sam zove **nepromijenjenim** (`04` §7.5) ostaju mjerodavne, pa je kanonski izvršni redoslijed:
...
## P5-I4C FORMAL CLOSURE ANNOTATION — D-080 (2026-09-05)

**Ovo je ne-checkbox anotacija.** Ona **ne dodaje nijedan checklist red** i **ne mijenja nijednu
kućicu**; markeri checkboxa se u njoj **namjerno ne koriste**. **Nijedan pasus, blok ni anotacija
iznad se ne prepisuje.**

- **D-080 je post-merge pomirenje i formalno zatvaranje pod-gatea `P5-I4C`.** `P5-I4C` je
  **implementiran, vlasnički pregledan, prihvaćen, objavljen kroz PR #59, merged kao `533b73bb…`,
  kanonski i publikaciono verifikovan**; zasebna higijenska ispravka sirovog `NUL`-a objavljena je
  kroz **PR #60** i kanonska je kao **`c5a778b6…`**. **Publikacioni drift je nula na obje trake.**
  **Dokumentacija je isključivo**; **nijedna linija koda, testa, scheme, migracije, granta,
  zavisnosti ni lockfilea se ovom odlukom ne uvodi**, i **nijedna baza nije kontaktirana**.
- **Vlasnički closure uslovi:** `OD-P5-I4C-CLOSE-1` (nezavisan review mora prethoditi) =
  **`SATISFIED`**; `OD-P5-I4C-CLOSE-2` (higijenski nalaz sirovog `NUL`-a mora biti riješen i
  kanonski prije zatvaranja) = **`SATISFIED`**.
- **Autorizovan izvršivi obuhvat D-079 `RULING B` je ispunjen u cijelosti** — svih **trinaest**
  stavki: idempotency servis; advisory-lock kontrola konkurencije; audit writer;
  `POST /patient-references`; konzumacija `MANUAL` v1 normalizacije; perzistencija HMAC-a kao
  `external_patient_ref_hash`; ciljani retry pseudonima; `PATIENT_REFERENCE_ALREADY_EXISTS`;
  servisni lookup po pseudonimu; servisni lookup po eksternoj referenci; `CO-P5-I3-I4-1`;
  `CO-P5-I3-I4-2`; integracioni / sigurnosni / API / concurrency dokazi. **Obuhvat nije proširen.**
- **Prihvaćeni verifikacijski dokaz `P5-I4C`:** `typecheck` `PASS`, `lint` `PASS`, unit
  `49 / 1259`, e2e `5 / 41`, integration `4 / 46`, security `23 / 851`; agregat **81 fajl / 2197
  testova / 2197 `PASS` / 0 padova / 0 preskoka**. **Zasebna higijenska verifikacija:** pogođeni
  spec `62 / 62 PASS`, ciljani `P5-I4C` set `154 / 154 PASS`, puna unit svita `1259 / 1259 PASS`,
  `typecheck` `PASS`, `lint` `PASS`, **diferencijalni** format `PASS`. **Agregat `81 / 2197` NIJE
  ponovo izvršen nakon higijene**, i **e2e / integration / security trake NISU rerunovane tokom
  higijenskog gatea.**
- **Dispozicija nalaza `P5-I4C` reviewa:** `F-1`, `F-2`, `F-3`, `F-4` = **`INFORMATIONAL` /
  `ACCEPTED` / `NON-BLOCKING`**;
  **`ERROR_CODE_MIRROR_CONFORMANCE_RESTORATION_CONFIRMED = CONFIRMED`**; `P5-I4C` sirovi `NUL` =
  **`RESOLVED` / `CANONICAL` / `NON-BLOCKING`**; sirovi `NUL` u
  `apps/api/src/identity/domain/permission-matrix.ts` = **`PRE-EXISTING` / `OUT OF P5-I4C SCOPE` /
  `UNCHANGED` / `SEPARATE NON-BLOCKING FOLLOW-UP`**, **bez remedijacije u ovom gateu**.
  **`BLOCKING FINDINGS OPEN = 0`.**
- **Anotacije D-072, D-074, D-076, D-077, D-078 i D-079 iznad opisuju pred-D-080 stanje** —
  „`P5-I4C IMPLEMENTATION AUTHORIZED = NO`", „`P5-I4C IMPLEMENTATION STARTED = NO`" i
  „`POST /patient-references` nije implementiran" — **historijski su tačne** i **ne prepisuju se**;
  **mjerodavan je ovaj statusni model.**
- **Formalno zatvaranje nije odmah efektivno** — postaje efektivno tek nakon što D-080 sam bude
...
- **Nijedna kućica se ovim zatvaranjem ne označava.** Kanonsko pravilo ostaje: **pod-gate
  `P5-I4A` / `P5-I4B` / `P5-I4C` ne prevodi nijedan red roditeljskog checklista `P5-I4`.**
  Sedamnaest forecast redova prevodi **isključivo** zaseban gate zatvaranja roditeljskog `P5-I4`.
- **`CO-P5-I3-I4-1` i `CO-P5-I3-I4-2` su ispunjeni obuhvatom `P5-I4C`**, ali ostaju **ne-checkbox
  kriteriji prihvatanja** roditeljskog `P5-I4`; **ne dobijaju vlastite redove** ni sada ni kasnije,
  i **njihova ispunjenost sama po sebi ne prevodi nijednu kućicu.**
- **`P5-I4C` je treći i posljednji pod-gate; `P5-I4D` ne postoji.** Naredni lifecycle gate nakon
  efektivnog zatvaranja `P5-I4C` je **zaseban gate formalnog zatvaranja roditeljskog `P5-I4`** —
  **ne `P5-I5`**.
...
EXPECTED_POST_P5_I4_CLOSURE_CHECKLIST  = 49 / 31   (ISKLJUCIVO FORECAST, NIJE TEKUCE STANJE)
```

```text
P5-I4C  CANONICAL / FORMALLY CLOSED (efektivno tek po publikaciji D-080)
...
**Zatvaranje pod-gatea nije zatvaranje roditelja.** Zatvaranje `P5-I4C` **NE zatvara automatski
`P5-I4`**, **NE prevodi automatski sedamnaest roditeljskih forecast redova** i **NE odblokira
